// Call one connector tool from the command line: npx tsx scripts/call-tool.ts <tool> '<json args>' [image.png]
// An image result is saved to the given file.
import { Client } from "@modelcontextprotocol/sdk/client/index.js";
import { StreamableHTTPClientTransport } from "@modelcontextprotocol/sdk/client/streamableHttp.js";
import fs from "node:fs";

const [tool, json = "{}", imageFile] = process.argv.slice(2);
...
for (const c of result.content as { type: string; text?: string; data?: string }[]) {
  if (c.type === "image" && imageFile && c.data) {
    fs.writeFileSync(imageFile, Buffer.from(c.data, "base64"));
    console.log(`Saved ${imageFile}`);
  } else console.log(c.type === "text" ? c.text : `[${c.type}: ${c.data?.length ?? 0} base64 chars]`);
}
await client.close();
